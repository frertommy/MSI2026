/**
 * Oracle V3 — Server Component
 *
 * Same layout as V2 but reads from team_oracle_v3_state.
 * Settlement rows filtered to oracle_version='v3'.
 * Adds bt_std_error column for BT solver diagnostics.
 */
...
// ─── Server component ──────────────────────────────────────
export default async function OracleV3Page() {
  // Fetch V3 state from team_oracle_v3_state
  const teamStatesPromise = fetchAll<TeamOracleRow>(
    "team_oracle_v3_state",
    "team_id, season, B_value:b_value, M1_value:m1_value, published_index, confidence_score, bt_std_error, next_fixture_id, last_market_refresh_ts, updated_at"
  );

  // Fetch V3-only settlements
      ...
      <header className="border-b border-border px-6 py-4">
        <div className="mx-auto max-w-7xl flex items-center gap-4">
          ...
          <span className="text-xs text-cyan-400 font-mono">
            bradley-terry MAP &middot; &gamma;=0.08
          </span>
        ...
        {teamCount === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            <div className="text-muted text-sm font-mono mb-2">
              Oracle V3 not yet active
            </div>
            <div className="text-muted/60 text-xs font-mono max-w-md">
              Run the reseed script first: cd scheduler && npx tsx src/scripts/reseed-v3.ts
              Then set ORACLE_V3_ENABLED=true.
            </div>
          </div>
        ) : (
          <OracleV3Client
