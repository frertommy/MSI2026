...
export function OracleV3Client({ teamStates, settlements, matches }: Props) {
  ...
  const fetchMatchPriceHistory = useCallback(async (match: MatchRow) => {
    ...
    try {
      const res = await fetch(
        `/api/v3/match-price-history?fixture_id=${match.fixture_id}`
      );
      if (res.ok) {
  ...
  const tableRows = useMemo(() => {
    ...
    for (const ts of teamStates) {
      ...
      rows.push({
        ...
        bt_std_error: ts.bt_std_error != null ? Number(ts.bt_std_error) : null,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border border-cyan-500/30 rounded-lg p-3 bg-cyan-950/20 text-xs font-mono text-muted">
        <span className="text-cyan-400 font-bold">Oracle V3:</span>{" "}
        bradley-terry MAP &middot; &gamma;=0.08 &middot; Index = B + M1
      </div>
