...
export interface V2SettlementRow {
...
export function DiagnosticsV2Client({
  settlements,
  krSnapshots,
  teamStates,
  priceHistory,
    ...
    <div className="space-y-8">
      ...
      <section>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
          ...
          <Stat label="V2 Settlements" value={String(settlements.length)} />
      ...
      {/* ── V2 PARAMETERS ──────────────────────────────────── */}
      <section>
        <SectionHeader title="Oracle V2 Parameters" />
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
          <Stat label="K (Settlement)" value="30" sub="ΔB = K(S-E_KR) + γ(R-B)" />
          <Stat label="Gravity γ" value="0.05" sub="hidden in settlement" />
          <Stat label="M1 Clamp" value="±120" />
          <Stat label="Horizon Days" value="21" />
          <Stat label="Home Advantage" value="65 Elo" />
          <Stat label="c_books Full At" value="5 books" />
        </div>
      </section>

      {/* ── GRAVITY ANALYSIS (V2-specific) ─────────────────── */}
      <section>
        <SectionHeader title="Gravity Analysis" sub="V2-specific · γ × (R_market − B)" />
        {!hasSettlements ? (
          <div className="border border-border rounded-lg p-8 text-center">
            <div className="text-2xl mb-2">&#9881;</div>
            <div className="text-sm text-muted font-mono">No V2 settlements yet</div>
            <div className="text-[10px] text-muted/50 font-mono mt-1">Gravity data will appear after the first V2 settlement</div>
          </div>
        ) : (
          <>
      ...
      <section>
        ...
        {!hasSettlements ? (
          <div className="border border-border rounded-lg p-6 text-center text-sm text-muted font-mono">
            No settlements yet — check back after March 20
          </div>
        ) : (
          <>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
