...
export function OracleV2Client({ teamStates, settlements, matches }: Props) {
  ...
  const fetchPriceHistory = useCallback(async (team: string) => {
    ...
    try {
      const res = await fetch(`/api/v2/price-history?team=${encodeURIComponent(team)}`);
      if (res.ok) {
  ...
  const fetchMatchPriceHistory = useCallback(async (match: MatchRow) => {
    ...
    try {
      const res = await fetch(
        `/api/v2/match-price-history?fixture_id=${match.fixture_id}`
      );
      if (res.ok) {
  ...
  const tableRows = useMemo(() => {
    ...
    for (const ts of teamStates) {
      ...
      const lastGravity = teamSettlements.length > 0 ? Number(teamSettlements[0].gravity_component) : null;
  ...
  const selectedData = useMemo((): ChartPoint[] | null => {
    ...
    for (const ph of rows) {
      ...
      if ((ph.publish_reason === "settlement_v2" || ph.publish_reason === "settlement") && ph.source_fixture_id != null) {
        const match = matchById.get(ph.source_fixture_id);
        date = match ? match.date.slice(0, 10) : ph.timestamp.slice(0, 10);
      } else if (ph.publish_reason === "market_refresh_v2" || ph.publish_reason === "market_refresh" || ph.publish_reason === "live_update") {
        date = ph.timestamp.slice(0, 10);
      } else if (ph.publish_reason === "bootstrap_v2" || ph.publish_reason === "bootstrap") {
  ...
  return (
    <div className="space-y-6">
      {/* Spec summary */}
      <div className="border border-cyan-500/30 rounded-lg p-3 bg-cyan-950/20 text-xs font-mono text-muted">
        <span className="text-cyan-400 font-bold">Oracle V2:</span>{" "}
        &Delta;B = K &times; (S &minus; E_KR) + &gamma; &times; (R_mkt &minus; B) &middot; K=30 &middot; &gamma;=0.05 &middot; Index = B + M1
      </div>
                      ...
                      <div style={tooltipStyle} className="p-2">
                        ...
                        {!pt.result && (
                          <div className="text-muted text-[10px]">
                            {pt.publish_reason.replace("_v2", "") === "market_refresh" || pt.publish_reason === "live_update"
                              ? "M1 update"
                              : pt.publish_reason.replace("_v2", "")}
                          </div>
                        )}
