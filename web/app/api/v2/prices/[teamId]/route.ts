...
/**
 * GET /api/v2/prices/[teamId]
 *
 * Returns current price + recent history + recent settlements for a single team.
 * Query params:
 *   history (optional): hours of price history, default 24, max 168 (1 week)
 *
 * Notes:
 *   - oracle_price_history column is "team" (not "team_id")
 *   - settlement_log PK is "settlement_id" (not "id")
 *   - V2 settlements (oracle_version='v2') may not exist yet — returns empty array
 */
...
const V2_REASONS = ["bootstrap_v2", "settlement_v2", "market_refresh_v2"];
...
export async function GET(
  ...
  try {
    ...
    // Fetch current state from team_oracle_v2_state
    ...
    // Determine league from oracle_price_history (most recent row for this team)
    ...
    // Fetch history and settlements in parallel
    ...
    // Downsample history to ~1 row per hour if large
    ...
    console.error("v2/prices/[teamId] unexpected error:", err);
    return NextResponse.json(
...
async function fetchHistory(
  ...
  while (true) {
    const { data, error } = await supabase
      ...
      .eq("team", team) // column is "team", not "team_id"
    ...
    if (error) {
      console.error(
        "v2/prices/[teamId] history fetch error:",
        error.message
      );
      break;
    }
...
async function fetchSettlements(team: string): Promise<SettlementRow[]> {
  // oracle_version='v2' settlements may not exist yet — returns empty array
  const { data, error } = await supabase
    .from("settlement_log")
  ...
  if (error) {
    console.error(
      "v2/prices/[teamId] settlement fetch error:",
      error.message
    );
    return [];
  }
...
function downsampleHourly(rows: PriceHistoryRow[]): PriceHistoryRow[] {
  ...
  for (const row of rows) {
    ...
    // Keep the latest row per hour bucket
