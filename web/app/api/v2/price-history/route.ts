...
/**
 * GET /api/v2/price-history?team={teamId}
 *
 * Returns oracle_price_history rows for a single team (V2 reasons only).
 * market_refresh_v2 rows are deduplicated server-side: only the LATEST per date is kept.
 */
export async function GET(req: NextRequest) {
  ...
  // V2 publish reasons
  const nonRefreshReasons = ["bootstrap_v2", "settlement_v2", "live_update"];
  const refreshReasons = ["market_refresh_v2"];
  ...
  // Deduplicate market_refresh_v2: keep only the LATEST row per date
...
async function fetchPaginated(
  ...
  while (true) {
    ...
    if (error) {
      console.error(`v2 price-history fetch error:`, error.message);
      break;
    }
