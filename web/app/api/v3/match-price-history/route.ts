...
/**
 * GET /api/v3/match-price-history?fixture_id=123
 *
 * Returns V3 oracle_price_history for both teams in a match,
 * filtered to a 48h window around kickoff.
 * market_refresh_v3 rows are downsampled to 1 per 15-min bucket.
 */
...
async function fetchTeamWindow(
  ...
  while (true) {
    ...
    if (error) {
      console.error(`v3 match-price-history fetch error for ${team}:`, error.message);
      break;
    }
...
function downsample(rows: PriceHistoryRow[]): PriceHistoryRow[] {
  ...
  for (const row of rows) {
    if (row.publish_reason !== "market_refresh_v3") {
      keep.push(row);
      continue;
    }
