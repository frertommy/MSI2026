...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error("v3/matches unexpected error:", err);
    return NextResponse.json(
...
async function fetchUpcomingMatches(
  ...
  if (error) {
    console.error("v3/matches upcoming fetch error:", error.message);
    return [];
  }
...
async function fetchKRSnapshots(
  ...
  if (error) {
    console.error("v3/matches KR fetch error:", error.message);
    return new Map();
  }
...
async function fetchTeamState(
  ...
  if (error) {
    console.error("v3/matches state fetch error:", error.message);
    return new Map();
  }
...
async function fetchSettledMatches(
  ...
  // Get recent V3 settlements from settlement_log
  // Note: oracle_version='v3' may have 0 rows if V3 settlement hasn't run yet
