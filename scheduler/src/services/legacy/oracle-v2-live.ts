/**
 * oracle-v2-live.ts — Live layer (L) for the V2 Oracle.
 *
 * Ported from oracle-v1-live.ts — identical logic, uses ORACLE_V2_K.
 *
 * L(t) = K × (E_live(t) − E_KR)
 *
 * Where:
 ...
 *   - Uses power de-vig + median consensus (same as freezeKR and refreshM1)
...
import { ORACLE_V2_K } from "../config.js";
 ...
 */
export async function computeLiveLayerV2(
  fixtureId: number,
  teamId: string,
  isHome: boolean
  ...
  // If no KR exists yet (match just kicked off), freeze it now
  const frozenKR = await freezeKR(fixtureId);

  if (!frozenKR) {
    log.warn(`V2 Live layer: fixture ${fixtureId} — no KR available, L=0`);
    return {
      L: 0,
      E_live: null,
      E_KR: 0,
  ...
  if (matchErr || !matchData) {
    log.error(`V2 Live layer: fixture ${fixtureId} — match not found: ${matchErr?.message ?? "no data"}`);
  ...
  if (oddsErr) {
    log.error(`V2 Live layer: odds query failed for fixture ${fixtureId}: ${oddsErr.message}`);
      ...
      if (altFixtures && altFixtures.length > 0) {
        for (const alt of altFixtures) {
          ...
          if (altOdds && altOdds.length > 0) {
            ...
            log.info(
              `V2 Live layer fallback: fixture ${fixtureId} had 0 live odds, using alt ${altId} (${allSnapshots.length} bookmakers)`
            );
            break;
          }
        }
  ...
  if (freshBooks.size < 2) {
    log.warn(
      `V2 Live layer: fixture ${fixtureId} team=${teamId} — ` +
  ...
  const L = ORACLE_V2_K * (E_live - E_KR);
