/**
 * oracle-v1-live.ts — Live layer (L) for the V1 Oracle.
 *
 * Exports one function: computeLiveLayer(fixtureId, teamId, isHome)
 *
 * L(t) = K × (E_live(t) − E_KR)
 *
 * Where:
 ...
 *   K          = 30 (same K factor as settlement, from ORACLE_V1_K)
 *
 * During a live match:
 ...
 *   - No imports from pricing-engine.ts or oracle-v1-market.ts
 */
 ...
 */
export async function computeLiveLayer(
  fixtureId: number,
  teamId: string,
  isHome: boolean
  ...
  if (!frozenKR) {
    log.warn(`Live layer: fixture ${fixtureId} — no KR available, L=0`);
    return {
      L: 0,
      E_live: null,
      E_KR: 0,
  ...
  if (matchErr || !matchData) {
    log.error(`Live layer: fixture ${fixtureId} — match not found: ${matchErr?.message ?? "no data"}`);
  ...
  const L = ORACLE_V1_K * (E_live - E_KR);
